...
def compute_align_loss(model, desc_enc, example):
    ...
    rel_cols_t = torch.LongTensor(
        sorted(list(set(rel_cols))),
    ).to(model._device)
    rel_tabs_t = torch.LongTensor(
        sorted(list(set(rel_tabs))),
    ).to(model._device)
    ...
    un_rel_cols_t = torch.LongTensor(
    ...
    mc_margin = torch.log(mc_max_unrel_att).mean() - \
        torch.log(mc_max_rel_att).mean()

    t_num = desc_enc.m2t_align_mat.size()[1]
    if t_num > len(set(rel_tabs)):
        un_rel_tabs_t = torch.LongTensor(
            sorted(list(set(range(t_num)) - set(rel_tabs))),
        ).to(model._device)
        mt_att_on_unrel_tab = desc_enc.m2t_align_mat.index_select(
            1, un_rel_tabs_t,
        )
        mt_max_unrel_att, _ = mt_att_on_unrel_tab.max(dim=0)
        mt_max_unrel_att.clamp_(min=1e-9)
        mt_margin = torch.log(mt_max_unrel_att).mean() - \
