...
@registry.register("tokenizer", "DistilBERTTokenizer")
class DistilBERTTokenizer(AbstractTokenizer):
    def __init__(self, pretrained_model_name_or_path: str):
        self._distilbert_tokenizer = DistilBertTokenizerFast.from_pretrained(
            pretrained_model_name_or_path=pretrained_model_name_or_path
        )

    def tokenize(self, s: str) -> List[str]:
        return self._distilbert_tokenizer.tokenize(s)

    def tokenize_with_raw(self, s: str) -> List[Tuple[str, str]]:
        # TODO: at some point, hopefully, transformers API will be mature enough
        # to do this in 1 call instead of 2
        tokens = self._distilbert_tokenizer.tokenize(s)
        encoding_result = self._distilbert_tokenizer(s, return_offsets_mapping=True)
        assert len(encoding_result[0]) == len(tokens) + 2
        raw_token_strings = [
            s[start:end] for start, end in encoding_result["offset_mapping"][1:-1]
        ]
    ...
    def convert_token_to_id(self, s: str) -> int:
        return self._distilbert_tokenizer.convert_tokens_to_ids(s)

    @property
    def cls_token(self) -> str:
        return self._distilbert_tokenizer.cls_token

    @property
    def sep_token(self) -> str:
        return self._distilbert_tokenizer.sep_token
