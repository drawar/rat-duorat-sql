...
class DistilBertEncoder(InitialEncoder):
    def __init__(
        self,
        pretrained_model_name_or_path: str,
        trainable: bool,
        ...
        use_position_ids: bool,
        use_segments: bool,
        preproc: DistilBertDuoRATPreproc,
    ) -> None:
        super(DistilBertEncoder, self).__init__()

        assert isinstance(preproc, DistilBertDuoRATPreproc)
        ...
        if not trainable:
            for param in self.distilbert.parameters():
                param.requires_grad = False
        self.trainable = trainable
        assert 0 <= num_return_layers <= self.distilbert.config.num_hidden_layers + 1
        self.num_return_layers = num_return_layers

        self._bert_embed_dim = self.num_return_layers * self.distilbert.config.hidden_size
        if use_affine_transformation:
            self._embed_dim = embed_dim
            self.linear = nn.Linear(self._bert_embed_dim, self.embed_dim)
        else:
    ...
    def max_supported_input_length(self) -> Optional[int]:
        if self.use_position_ids or self.use_segments:
            ...
            return self.distilbert.config.max_position_embeddings
...
#         print(len(self.distilbert(
#             input_a.to(device=device),
#             attention_mask=_input_attention_mask,
# #             position_ids=None,
#         )))

        last_layer_hidden_state, all_hidden_states = self.distilbert(
            input_a.to(device=device),
            attention_mask=_input_attention_mask,
#             position_ids=_input_position_ids,
        )
...
#        print("\n\nall_hidden_states:", all_hidden_states)  # DEBUG
#        print("\n\nall_hidden_states:", type(all_hidden_states))  # DEBUG
#        print("\n\n\nself.distilbert.config.num_hidden_layers:", self.distilbert.config.num_hidden_layers)  # DEBUG



        assert len(all_hidden_states) == self.distilbert.config.num_hidden_layers + 1
        # assert all(
        #     hidden_state.dtype == torch.float for hidden_state in all_hidden_states
        # )
        assert all(
            hidden_state.shape
            == (batch_size, max_input_length, self.distilbert.config.hidden_size)
            for hidden_state in all_hidden_states
        )
        assert all(hidden_state.device == device for hidden_state in all_hidden_states)
        assert all_hidden_states[-1].data_ptr() == last_layer_hidden_state.data_ptr()
