...
def load_tables(paths):
    ...
    for path in paths:
        for line in open(path):
            ...
            schemas[db_id] = spider.Schema(
                db_id, tables, columns, foreign_key_graph, None,
            )
...
class WikiSqlDataset(torch.utils.data.Dataset):
    ...
    class Metrics:
        ...
        def _evaluate_one(self, item, inferred_code):
            ...
            gold_result = self.db_engine.execute_query(
                item.schema.db_id, gold_query, lower=True,
            )

            pred_query = None
            pred_result = None
            try:
                pred_query = query.Query.from_dict(
                    inferred_code, ordered=False,
                )
                pred_result = self.db_engine.execute_query(
                    item.schema.db_id, pred_query, lower=True,
                )
            except:
