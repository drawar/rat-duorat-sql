...
class Inferer:
    ...
    def load_model(self, logdir, step):
        ...
        model = registry.construct(
            'model', self.config['model'], preproc=self.model_preproc, device=self.device,
        )
        ...
        if not last_step:
            raise Exception(
                f'Attempting to infer on untrained model in {logdir}, step={step}',
            )
        return model
    ...
    def infer(self, model, output_path, args):
        ...
        with torch.no_grad():
            if args.mode == 'infer':
                ...
                self._inner_infer(
                    model, args.beam_size, args.output_history, sliced_orig_data, sliced_preproc_data,
    ...
    def _inner_infer(
        self, model, beam_size, output_history, sliced_orig_data, sliced_preproc_data, output,
        use_heuristic=True,
    ):
        for i, (orig_item, preproc_item) in enumerate(
            ...
            decoded = self._infer_one(
                model, orig_item, preproc_item, beam_size, output_history, use_heuristic,
            )
            output.write(
                json.dumps({
                    'index': i,
