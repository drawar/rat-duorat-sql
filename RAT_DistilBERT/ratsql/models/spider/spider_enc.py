...
class SpiderEncoderBartPreproc(SpiderEncoderV2Preproc):
    ...
    def preprocess_item(self, item, validation_info):
        question = self._tokenize(item.text, item.orig['question'])
        preproc_schema = self._preprocess_schema(item.schema)
        question_bart_tokens = Bertokens(question)
        if self.compute_sc_link:
        ...
        if self.compute_cv_link:
            cv_link = question_bart_tokens.bart_cv_linking(item.schema)
        else:
            cv_link = {'num_date_match': {}, 'cell_match': {}}
...
class SpiderEncoderBart(torch.nn.Module):
    ...
    def __init__(
            ...
            preproc,
            update_config={},
            bart_token_type=False,
            bart_version='bart-base-uncased',
            summarize_header='first',
            use_column_type=True,
            include_in_memory=('question', 'column', 'table'),
    ):
        ...
        self._device = device
        self.preproc = preproc
        self.bart_token_type = bart_token_type
        self.base_enc_hidden_size = 1024 if bart_version == 'sshleifer/distilbart-xsum-12-6' else 768
        ...
        self.bart_model = BartForConditionalGeneration.from_pretrained(bart_version)
        self.tokenizer = self.preproc.tokenizer
        self.bart_model.resize_token_embeddings(
            len(self.tokenizer),
        )  # several tokens added
    ...
    def forward(self, descs):
        ...
        batch_id_map = {}  # some long examples are not included
        for batch_idx, desc in enumerate(descs):
            qs = self.pad_single_sentence_for_bart(desc['question'], cls=True)
            if self.use_column_type:
                cols = [
                    self.pad_single_sentence_for_bart(
                        c, cls=False,
                    ) for c in desc['columns']
                ]
            else:
            ...
            token_list = qs + [c for col in cols for c in col] + \
                [t for tab in tabs for t in tab]
            assert self.check_bart_seq(token_list)
            if len(token_list) > 512:
                long_seq_set.add(batch_idx)
                continue
        ...
        padded_token_lists, att_mask_lists, tok_type_lists = self.pad_sequence_for_bart_batch(
            batch_token_lists,
        )
        tokens_tensor = torch.LongTensor(padded_token_lists).to(self._device)
        att_masks_tensor = torch.LongTensor(att_mask_lists).to(self._device)

        if self.bart_token_type:
            tok_type_tensor = torch.LongTensor(tok_type_lists).to(self._device)
        ...
        for batch_idx, desc in enumerate(descs):
            ...
            if batch_idx in long_seq_set:
                ...
                bart_batch_idx = batch_id_map[batch_idx]
                q_enc = enc_output[bart_batch_idx][batch_id_to_retrieve_question[bart_batch_idx]]
                col_enc = enc_output[bart_batch_idx][batch_id_to_retrieve_column[bart_batch_idx]]
                tab_enc = enc_output[bart_batch_idx][batch_id_to_retrieve_table[bart_batch_idx]]

                if self.summarize_header == 'avg':
                    col_enc_2 = enc_output[bart_batch_idx][batch_id_to_retrieve_column_2[bart_batch_idx]]
                    tab_enc_2 = enc_output[bart_batch_idx][batch_id_to_retrieve_table_2[bart_batch_idx]]
    ...
    def encoder_long_seq(self, desc):
        """
        Since bart cannot handle sequence longer than 512, each column/table is encoded individually
        The representation of a column/table is the vector of the first token [CLS]
        """
        qs = self.pad_single_sentence_for_bart(desc['question'], cls=True)
        cols = [
        ...
        enc_q = self._bart_encode(qs)
        enc_col = self._bart_encode(cols)
        enc_tab = self._bart_encode(tabs)
        return enc_q, enc_col, enc_tab

    @DeprecationWarning
    def _bart_encode(self, toks):
        if not isinstance(toks[0], list):  # encode question words
            indexed_tokens = self.tokenizer.convert_tokens_to_ids(toks)
            tokens_tensor = torch.tensor([indexed_tokens]).to(self._device)
            outputs = self.bart_model(tokens_tensor)
            return outputs[0][0, 1:-1]  # remove [CLS] and [SEP]
        else:
            max_len = max(len(it) for it in toks)
            tok_ids = []
    ...
    def check_bart_seq(self, toks):
        if toks[0] == self.tokenizer.cls_token and toks[-1] == self.tokenizer.sep_token:
            return True
        else:
            return False

    def pad_single_sentence_for_bart(self, toks, cls=True):
        if cls:
            return [self.tokenizer.cls_token] + toks + [self.tokenizer.sep_token]
        else:
            return toks + [self.tokenizer.sep_token]

    def pad_sequence_for_bart_batch(self, tokens_lists):
        pad_id = self.tokenizer.pad_token_id
        max_len = max(len(it) for it in tokens_lists)
        assert max_len <= 512
        toks_ids = []
